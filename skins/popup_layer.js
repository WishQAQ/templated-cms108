...
var PopupLayer = new Class({
	...
	_init:function(options){
		...
		this.trigger.bind(this.options.eventType,function(){            //给触发元素绑定触发事件
			if(this.isSetPosition){
				this.setPosition(this.trigger.offset().left -400, this.trigger.offset().top + this.trigger.get(0).offsetHeight + this.options.offsets.y);
			}
	...
	doresize:function(){
		...
		if(this.isSetPosition){
			this.setPosition(this.trigger.offset().left, this.trigger.offset().top + this.trigger.get(0).offsetHeight + this.options.offsets.y);
		}
	},
